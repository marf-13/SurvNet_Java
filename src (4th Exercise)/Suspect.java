...
public class Suspect {
	...
	private ArrayList<Suspect> partners = new ArrayList<>(); //possiblePartners
	private ArrayList<Suspect> suggestedPartners = new ArrayList<>(); //suggestedPossiblePartners
	...
	/*Μέθοδος που ανατρέχει τους συνεργάτες των συνεργατών που έχει ο ύποπτος που αναζητήθηκε 
	 * και επιστρέφει προτεινόμενους συνεργάτες.
	 */
	public ArrayList<Suspect> getSuggestedPartners() {
		
		//Ψάχνουμε τους partners των partner του αρχικού μας υπόπτου.
		for(Suspect partner: partners) {
			for(Suspect suggestion: partner.partners) {
				if(!(partners.contains(suggestion) || suggestion.equals(this) || suggestedPartners.contains(suggestion))) { 
					//Αν δεν είναι ήδη partner με τον ύποπτο, δεν είναι ο αρχικός μας ύποπτος
					// και δεν υπάρχει ήδη ο partner στην λίστα, τότε προστίθεται.
					suggestedPartners.add(suggestion);
				}
			}
		}
